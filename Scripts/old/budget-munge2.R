...
df_orcamento <- df %>% 
  ...
  select(ugb_id:despesa_liquidada_via_directa_lvd) %>% 
  
  # Remove rows with missing CED code or where ALL dotação values are zero
  filter(
    !is.na(ced),
    !if_all(
      c(dotacao_inicial, dotacao_revista, dotacao_revista,
        dotacao_actualizada_da, dotacao_disponivel,
        dotacao_cabimentada_dc, ad_fundos_concedidos_af),
      ~ .x == 0
    )
  ) %>% 
